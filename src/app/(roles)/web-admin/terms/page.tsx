...
export default function TermsManagementPage() {
    ...
    const fetchTerms = async () => {
        setLoading(true);
        try {
            const res = await api.get("/public/terms-and-conditions");
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast.error(err.response?.data?.message || "Failed to update Terms & Conditions");
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="px-4 md:px-8 py-8 space-y-8 bg-[#F9FAFB] min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-[#0D1C35] font-sans">Terms & Conditions</h1>
                    <p className="text-sm text-gray-500 font-sans">Manage the legal terms with rich text formatting</p>
                </div>
                <button
                    onClick={handleSave}
            ...
            <div className="bg-white border border-gray-100 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 px-6 py-4 border-b border-gray-50 bg-gray-50/50">
                    <FileText size={16} className="text-[#0A47C2]" />
                    <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Terms Editor</span>
                </div>
