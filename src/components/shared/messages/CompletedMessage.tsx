    ...
    return (
        <div className={cn("flex items-start gap-3 max-w-2xl w-full mb-4", isMe && "flex-col items-end gap-1 ml-auto")}>
            ...
            <div className={cn("flex flex-col gap-1 w-full max-w-xs", isMe ? "items-end" : "items-start")}>
                <div className="bg-white rounded-3xl p-6 shadow-xl border border-[#0A47C2]/10 w-full space-y-6 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-2 opacity-5">
                       <ShieldCheck size={80} className="text-[#0A47C2]" />
                    </div>
                    
                    <h4 className="text-sm font-bold text-gray-700">
                        {msg.status === "PAID" ? "Offer Paid" : "Booking Completed"}
                    </h4>
                    ...
                    <div className="pt-2">
                        <div className="py-3 bg-blue-600 text-white font-bold text-[10px] rounded-2xl text-center uppercase tracking-widest shadow-lg shadow-blue-100 flex items-center justify-center gap-2">
                            <ShieldCheck size={14} />
                            {msg.status === "PAID" ? "Paid" : "Completed"}
                        </div>
                    </div>
                </div>
